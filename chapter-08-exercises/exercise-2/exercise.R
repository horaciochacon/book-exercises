...
sentence <- "Wow! That's a mouthful. In simpler words, Docker is a tool 
  that allows developers, sys-admins etc. to easily deploy 
  their applications in a sandbox (called containers) to run on the 
  host operating system i.e. Linux. The key benefit of Docker is that it 
  allows users to package an application with all of its dependencies into a 
  standardized unit for software development. Unlike virtual machines, containers 
  do not have high overhead and hence enable more efficient usage of the 
...
sentence_vec <- sentence_list[[1]]

# Use the `unique()` function to get a vector of unique letters

unique_letters <- unique(sentence_vec)
...
count_occurrences("a",sentence_vec)
...
as.list(sapply(unique_letters, count_occurrences, sentence_vec))
